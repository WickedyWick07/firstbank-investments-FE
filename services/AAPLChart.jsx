...
const AAPLChart = () => {
  ...
  const url = `https://api.polygon.io/v2/aggs/ticker/AAPL/range/1/day/${startDate}/${endDate}?adjusted=true&sort=asc&limit=5000&apiKey=${apiKey}`;
  ...
  if (loading) return <p style={{ color: '#fff' }}>Loading data for AAPL...</p>;
  if (error) return <p style={{ color: '#fff' }}>{error}</p>;

  return (
    <div className='bg-black rounded p-4'>
      <h3 className='text-primaryGold text-xl font-semibold uppercase mb-3'>Apple Stock</h3>
